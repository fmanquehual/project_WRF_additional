...
# 2
estadisticos_por_estacion <- function(matrix.i){
  ...
  db.k <- db.j[1:5,]
  idx <- as.numeric(row.names(db.k))
  rownames(db.k) <- 1:nrow(db.k)
  #db.k
  ...
  for (k in idx) {
    ...
    db.k$mejor.conf[fila.k] <- colnames(matrix.i)[id.k]
    db.k$mejor.valor[fila.k] <- round(matrix.i[k,id.k],2)
  }
...
#setwd('/home/msomos/Documentos/WRF/')
setwd('C:/Users/Usuario/Documents/Francisco/WRF/')

db.coor <- read.csv('coordenadas_estaciones_metereologicas.csv', dec=',')
head(db.coor)

est <- SpatialPointsDataFrame(coords=db.coor[,c('lon', 'lat')],data=db.coor, proj4string = CRS(wgs84))
plot(est, axes=TRUE)
...
setwd('C:/Users/Usuario/Documents/Francisco/WRF/Estadisticos/')

df1 <- read.csv('db_cdom.csv', dec = ',')
df2 <- read.csv('db_dga.csv', dec = ',')
df3 <- read.csv('db_dmc.csv', dec = ',')
df4 <- read.csv('db_ghcn.csv', dec = ',')
df5 <- read.csv('db_inia.csv', dec = ',')
df6 <- rbind(df1, df2, df3, df4, df5)
df <- subset(df6, lon != is.na(NA))
df$lon <- df$lon*-1
df$lat <- df$lat*-1
df
# fin ---
...
est.m1.PCBIAS <- estadisticos_por_estacion(m1.PCBIAS) ; est.m1.PCBIAS
est.m1.PCBIAS.pre.ok <- merge(est.m1.PCBIAS, df, by.x='est', by.y='names_modi')
...
est.m2.PCBIAS <- estadisticos_por_estacion(m2.PCBIAS) ; est.m2.PCBIAS
est.m2.PCBIAS.pre.ok <- merge(est.m2.PCBIAS, df, by.x='est', by.y='names_modi')
...
est.m3.PCBIAS.pre.ok <- merge(est.m3.PCBIAS, df, by.x='est', by.y='names_modi')
...
est.m4.PCBIAS.pre.ok <- merge(est.m4.PCBIAS, df, by.x='est', by.y='names_modi')
...
est.m5.PCBIAS.pre.ok <- merge(est.m5.PCBIAS, df, by.x='est', by.y='names_modi')
