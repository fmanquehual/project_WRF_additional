diagrama.de.Taylor.modificado.2 <- function (R.i, sd.ref.i, sd.model.i, add = FALSE, col = "red", pch = 19, pos.cor = TRUE,
                nombre.etiqueta=NULL, pos.x.nombre.etiqueta=0, pos.y.nombre.etiqueta=0, tamanho.nombre.etiqueta=1,
                xlab = "Standard deviation", ylab = "", main = "Taylor Diagram", tamanho.punto.de.referencia = 1,
                show.gamma = TRUE, ngamma = 3, gamma.col = 8, sd.arcs = 0, ancho.linea.de.pch=1,
                ...
                mar = c(4, 3, 4, 3), ...) 
{
  ...
  points(sd.f * R, sd.f * sin(acos(R)), pch = pch, col = col, 
         cex = pcex, lwd = ancho.linea.de.pch)
  text((sd.f * R)+pos.x.nombre.etiqueta , (sd.f * sin(acos(R)))+pos.y.nombre.etiqueta, labels = nombre.etiqueta,
       cex=tamanho.nombre.etiqueta)
  invisible(oldpar)
}
